...
e2e: $(SERVICE_IP)
	skaffold run
	kubectl rollout status --timeout 5m -w deployments/postgres-deployment
	kubectl rollout status --timeout 5m -w deployments/quotationservice
	kubectl rollout status --timeout 5m -w deployments/frontendservice
